...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text, word2ph, phone, tone, language_str, wav_path):
        ...
        # # added for aihub dataset
        # bert_path = bert_path.replace("TS", "TL")
        # bert_path = bert_path.replace("VS", "VL")

        try:
            bert = torch.load(bert_path)
            try:
                if not bert.shape[-1] == len(phone):
                    print(f"bert.shape[-1] is {bert.shape[-1]}")
                    print(f"len(phone) is {len(phone)}")
            except:
